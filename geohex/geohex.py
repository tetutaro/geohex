...
class GeoHex(object):
	...
	def __str__(self):
		...
		for i in range(len(self.hexes)):
			...
			info.append("  中心座標: (%f, %f)" % (i_hex['center'][0], i_hex['center'][1]))
			info.append("  右: (%f, %f)" % (i_hex['right'][0], i_hex['right'][1]))
			info.append("  右上: (%f, %f)" % (i_hex['t_right'][0], i_hex['t_right'][1]))
			info.append("  左上: (%f, %f)" % (i_hex['t_left'][0], i_hex['t_left'][1]))
			info.append("  左: (%f, %f)" % (i_hex['left'][0], i_hex['left'][1]))
			info.append("  左下: (%f, %f)" % (i_hex['b_left'][0], i_hex['b_left'][1]))
			info.append("  右下: (%f, %f)" % (i_hex['b_right'][0], i_hex['b_right'][1]))
		return '\n'.join(info)
